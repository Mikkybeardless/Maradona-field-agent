		...
		<div className='container mx-auto p-4'>
				...
				<tbody>
					{currentItems.map((request) => (
						<tr key={request.id}>
							...
							<td className={`py-2 px-4 border-b ${getStatusClass(request.status)}`}>{request.status}</td>
							<td className='py-2 px-4 border-b'>
								<Link href='/dashboard/inspection-details' className='text-blue-500 hover:underline'>
									View
								</Link>{' '}
								|{' '}
			...
			<div className='flex justify-between items-center mt-4'>
				...
				<div className='flex items-center space-x-6'>
					{/* Previous Button */}
					<button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1} className={`px-3 py-3 rounded-lg border ${currentPage === 1 ? 'opacity-70 border-grey/50 ' : 'border-orange text-black'}`}>
						<ArrowLeft2 size={14} />
					</button>
					{/* Page Numbers */}
					<div className='flex items-center gap-3'>
						{[...Array(totalPages)].map((_, i) => (
							<button key={i} onClick={() => paginate(i + 1)} className={`size-8 grid place-items-center rounded-full ${currentPage === i + 1 ? 'bg-orange text-white' : 'hover:bg-gray-100'}`}>
								{i + 1}
							</button>
						))}
					</div>
					{/* Next Button */}
					<button onClick={() => paginate(currentPage + 1)} disabled={currentPage === totalPages} className={`px-3 py-3 rounded-lg border ${currentPage === totalPages ? 'opacity-70 border-grey/50 ' : 'border-orange text-black'}`}>
						<ArrowRight2 size={14} />
					</button>
				</div>
			</div>
